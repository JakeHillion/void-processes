...
\todo{Complete performance study.}
...
Namespaces in Linux create varying levels of isolation by default. This is because many of the namespaces are generated in a copy-on-write fashion, meaning that changes are not immediate.

Mount namespaces were the first [CN] namespaces introduced to Linux, in kernel version X.Y.Z. The API is particularly unfriendly to creating a Void, as not only is the creation copy-on-write, many filesystems are mounted shared. This means that they propagate back through namespace boundaries. \todo{Finish section on mount namespaces}

Network namespaces \todo{Finish section on network namespaces}

\todo{Finish section on remaining namespaces}

\subsection{Creation in the emptiness}
